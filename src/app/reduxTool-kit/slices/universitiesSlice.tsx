...
export const getAllUniversities = createAsyncThunk("universitiesSlice/fetchUniversities", async (parameter: any) => {
  const { page, search, country, state, city, majors, fields, language, recommended, limit, userId, academic_degree, rating } = parameter;
  const { tokenMainSite } = parseCookies();

  const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/universities?limit=${limit || "10"}&searchTerm=${search || ""}&country_id=${country || ""}&state_id=${state || ""}&city_id=${city || ""}&major_id=${majors || ""}&field_id=${fields || ""}&page=${page || "1"}${recommended == "true" ? `&recommend=${recommended}` : ""}${userId ? `&user_id=${userId}` : ""}${academic_degree ? `&academic_degree=${academic_degree}` : ""}${rating ? `&rating=${rating}` : ""}&status=active`, {
    method: 'GET',
    headers: {
