...
function ProductUNI({ props, recommended }: {
  ...
  const subscribe = async (universityId: number) => {
    ...
    try {
      ...
      const result = await response.json();
      dispatch(getAllUniversities({ language, recommended: recommended, userId: user.id, limit: props }))
  ...
  useEffect(() => {
    ...
    if (!searchValue) {
      dispatch(getAllUniversities({ page: currentPage, limit: props, language, recommended: recommended, userId: user.id }));
    } else {
