...
import DistinguishCard from "./ui/DistinguishCard";
import useCurrentLang from "../_hooks/useCurrentLang";

type DistinguishItem = {
  titleKey: string;
  descKey: string;
  bgColor: string;
...
const DISTINGUISH_ITEMS: DistinguishItem[] = [
  {
    titleKey: "H3",
    descKey: "P3",
    bgColor: "#FE6C3F",
  ...
  return (
    <div className="relative ">
        ...
        >
          ...
          <div className="flex flex-col justify-between items-start  w-full">
            <div className="flex flex-col gap-4 mt-2">
              <div className="text-3xl font-bold text-white">{k("Head")}</div>
      ...
      <div className="grid gap-2 lg:gap-7 px-5 lg:px-16 xl:px-28 py-0 mt-0 grid-cols-[repeat(3,1fr)] max-md:grid-cols-[repeat(2,1fr)]  max-sm:grid-cols-[1fr] absolute top-96 ">
        ...
        {DISTINGUISH_ITEMS.map((item, index) => (
          <DistinguishCard
            key={index}
            title={k(item.titleKey)}
            description={k(item.descKey)}
