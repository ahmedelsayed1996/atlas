...
function IncompleteProfilePopup({ onClose }: CheckIsLoggedIn) {
    ...
    const handleUpdatePhoneNumber = async () => {
        ...
        // console.log("user phoneNumber", user?.phone_number);
        ...
        formDataBuildIn.append("phone_number", phoneNumber ? `+${phoneNumber}` : user?.phone_number);
        // formDataBuildIn.append("phone_number", user?.phone_number || `+${phoneNumber}`);
        try {
    ...
    return (
        <div className="flex fixed top-0 left-0 justify-center items-center bg-black bg-opacity-75 size-full z-30">
            <div className="flex flex-col p-8 my-36 max-w-full bg-white rounded-2xl border border-gray border-solid shadow-2xl w-[700px] lg:w-[786px] max-md:px-5 max-md:mt-10 mx-4 md:mx-10">
                ...
                <form onSubmit={handleSubmit} className='flex flex-col  items-end justify-end'>
                    {step === "one" ? <>
                        ...
                        {isLoading ? <div className='w-1/3 pt-10'><Spinner /></div> :
                            <div
                                className="self-end text-center py-3 mt-10 text-white bg-primary rounded-3xl border capitalize cursor-pointer w-1/4"
                                onClick={() => {
