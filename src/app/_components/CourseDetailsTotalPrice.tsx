...
import { clearCourse } from "../reduxTool-kit/slices/courseSlice";
...
export default function CourseDetailsTotalPrice({ dataOfcourse, onClose }: pro) {
   ...
   return (
      <>
         ...
         <aside id="filters" className="flex flex-col w-[40%] order-2 md:order-1 max-md:w-full bg-white rounded-3xl border border-solid border-zinc-100 p-6
         min-w-80 z-10
         ">
            <form onSubmit={(e) => createQoute(e)}>
               {/* Heading */}
               <div className="flex items-center justify-between py-1">
                  <span className="font-bold text-[clamp(1rem,1.80vw,3rem)]">{tr("totalReservation")}</span>
               ...
               <div className="border-y border-[#eee] pt-6 pb-4 flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                     ...
                     <div className="bg-[#fafafa] border border-[#eee] rounded-3xl py-1 px-4 text-ellipsis flex gap-2.5 items-center justify-between">
                        <div className="w-full bg-transparent  cursor-pointer">{InfoForOrder?.course?.name}</div>
                        <div className=" my-auto text-black font-bold cursor-pointer" onClick={() => {
                           ...
                           if (el) {
                              // optional: if you have a fixed header
                              const header = document.getElementById("siteHeader");
                              const headerH = header?.offsetHeight ?? 0;

                              // mobile browser UI offset (iOS/Android)
                              const vpOffset = window.visualViewport?.offsetTop ?? 0;

                              const y = el.getBoundingClientRect().top + window.scrollY - headerH - 72; // 5px gap
                              window.scrollTo({ top: y - vpOffset, behavior: "smooth" });
                           }
                        }}>
                           <svg className=" rotate-45" width="20" height="20" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                              <path d="M7 11H15M11 7V15M21 11C21 16.5228 16.5228 21 11 21C5.47715 21 1 16.5228 1 11C1 5.47715 5.47715 1 11 1C16.5228 1 21 5.47715 21 11Z" stroke="#365D8D" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                           </svg>
                        </div>
                     </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-2">
                     ...
                     <div className="bg-[#fafafa] border border-[#eee] rounded-3xl py-1 px-4 shadow-sm">
                  ...
                  <div className="flex flex-col gap-2">
                     ...
                     <div className="bg-[#fafafa] border border-[#eee] rounded-3xl py-1 px-4 shadow-sm">
                        <select onChange={(e: any) => {
                           ...
                           name="courseType" id="courseType" className="bg-transparent w-full focus:outline-none text-nowrap  whitespace-nowrap">
                           ...
                           {InfoForOrder?.branch?.residence?.map((acc: Accommodation, index: number) => {
                              return <option key={index} value={JSON.stringify(acc)}
                                 title={`${acc.accommodationName}-${acc.roomCategory}-${acc.accommodationFees} ${InfoForOrder.branch.currency}`}>
                                 -  {`${acc.accommodationName?.slice(0, 15)}-${acc.roomCategory?.slice(0, 15)}-${acc.accommodationFees} ${InfoForOrder.branch.currency}`}
                              </option>
                           })}
                        </select>
                     </div>
                  ...
                  <div className="flex  gap-10 justify-between items-start px-5 py-3 rounded-lg  bg-[#FAFAFA] shadow-sm cursor-pointer"
                     ...
                     {healthInsurance ? (
                        ...
                        >
                           <svg className="border border-gray rounded" width="20" height="20" viewBox="0 0 20 20" fill="#DADCE0" xmlns="http://www.w3.org/2000/svg">
                              <path d="M0 2C0 0.895431 0.895431 0 2 0H18C19.1046 0 20 0.895431 20 2V18C20 19.1046 19.1046 20 18 20H2C0.895431 20 0 19.1046 0 18V2Z" fill="white" />
                              {/* <path fillRule="evenodd" clipRule="evenodd" d="M14.662 6.27936C15.1127 6.72997 15.1127 7.46056 14.662 7.91117L9.37358 13.1997C9.1572 13.4161 8.86371 13.5376 8.55769 13.5376C8.25167 13.5376 7.95819 13.4161 7.7418 13.1997L5.33795 10.7958C4.88735 10.3452 4.88735 9.61462 5.33795 9.16401C5.78856 8.7134 6.51913 8.7134 6.96974 9.16401L8.55769 10.752L13.0303 6.27936C13.4809 5.82875 14.2114 5.82875 14.662 6.27936Z" fill="white" /> */}
                           </svg>
                        </div>
                     )}
               ...
               <div className="flex flex-col gap-1 border-t border-t-[#eee]">
                           ...
                           <div className="flex flex-wrap gap-10 justify-between items-center mt-1 w-full text-start max-md:max-w-full">
                              <div className="self-stretch my-auto text-sm font-medium text-zinc-500">
                                 <p className="text-[#CAA313] flex items-center gap-1 text-sm w-auto">
                                    ...
                                    {/* {tu("commentEduxa")} */}
                                    {course?.translation?.offer_comment}
                                 </p>
                              </div>
                           </div>
                  ...
                  <div className="flex justify-between items-center py-2">
                     ...
                     <span className="font-bold">{getAllPrice.residence.totalPrice} {getAllPrice.totals.currency.toUpperCase()}</span>
                  </div>
                  {/* Health Insurance */}
                  <div className="flex justify-between items-center py-2">
                  ...
                  <div className="flex gap-1 items-start">
                     <input className=" w-4 h-4 border border-zinc-300 rounded accent-amber-500 text-white pt-2" type="checkbox" name="agree" id="agree" checked={agreed} onChange={(e) => {
