...
// import { getLanguageStudy } from "../reduxTool-kit/slices/languageStudySlice";
...
//   const [isAcademicDegreeOpen, setIsAcademicDegreeOpen] = useState<boolean>(false);
//   const [selectedAcademicDegree, setSelectedAcademicDegree] = useState<string>("");
//   const [isLanguageOpen, setIsLanguageOpen] = useState<boolean>(false);
//   const [selectedLanguage, setSelectedLanguage] = useState<selectLanguageStudy>();
...
//   const langLanguageStudyRef = useRef("");
...
//   const languageStudy = useSelector((state: any) => state.languageStudy);
...
//   );
//   const filteredLanguageStudy = languageStudy?.languageStudy?.data?.filter((langStudy: any) =>
//     langStudy.name.toLowerCase().includes(searchLanguage.toLowerCase())
//   );
...
//       search: searchValue,
//       languageStudy: selectedLanguage?.id,
//     };
...
//               {/* Filter By  Text */}
//               <div className="flex flex-col gap-2 items-start lg:w-72  xl:flex-1">
//                 {/* <div className="text-base font-bold tracking-normal leading-7 text-slate-900 max-sm:text-sm">
//                   {n("searchButton")}
//                 </div> */}
//                 <div className="flex gap-4 items-center px-4 py-4 h-10 border border-solid bg-neutral-50 border-zinc-100 rounded-[64px] max-sm:px-4 w-full">
//                   <div>
//                     <svg
//                       width="20"
//                       height="20"
...
//                   </span>
//                   <svg
//                     className={`transition-transform duration-500 ease-in-out ${isAcademicDegreeOpen ? "rotate-180" : ""} `}
//                     width="24"
//                     height="24"
//                     viewBox="0 0 24 24"
//                     fill="none"
...
//                   }}
//                   className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100 h-10 rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3 "
//                 // className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100  rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3 "
//                 >
//                   <span className="text-base tracking-wide text-neutral-600 max-sm:text-sm">
//                     {selectedCountry?.name || n("searchFiltersDrd1")}
//                   </span>
...
//                   </span>
//                   <svg
//                     className={`transition-transform duration-500 ease-in-out ${isCityOpened ? "rotate-180" : ""} `}
//                     width="24"
//                     height="24"
//                     viewBox="0 0 24 24"
//                     fill="none"
...
//                     const currentCountryLang = langLanguageStudyRef.current !== language;
//                     if (!languageStudy?.languageStudy?.data?.length || currentCountryLang) {
//                       dispatch(getLanguageStudy({ language: language }));
//                       langLanguageStudyRef.current = language;
//                     }
...
//                 >
//                   <span className="text-base tracking-wide text-neutral-600 max-sm:text-sm">
//                     {selectedLanguage?.name || n("searchFiltersDrd4+")}
//                   </span>
//                   <svg
//                     className={`transition-transform duration-500 ease-in-out ${isLanguageOpen ? "rotate-180" : ""} `}
//                     width="24"
//                     height="24"
//                     viewBox="0 0 24 24"
//                     fill="none"
...
//                     {/* قائمة النتائج بعد الفلترة */}
//                     {!languageStudy.loading ? (
//                       filteredLanguageStudy?.length > 0 ? (
//                         filteredLanguageStudy.map((country: any) => (
//                           <div
//                             key={country?.id}
//                             onClick={() => {
...
import { getLanguageStudy } from "../reduxTool-kit/slices/languageStudySlice";
import { selectCountries } from "../reduxTool-kit/selectors/countriesSelectors";
import { selectStates } from "../reduxTool-kit/selectors/stateSelectors";
import { selectCities } from "../reduxTool-kit/selectors/citiesSelectors";
import { selectMajors } from "../reduxTool-kit/selectors/majorsSelectors";
import { selectLanguageStudy } from "../reduxTool-kit/selectors/languageStudySelectors";
import { City, Country, State } from "../types/country";
import { Majors } from "../types/major";
import { LanguageStudy } from "../types/languageStudy";
interface SelectedValues {
  country: Country | null;
  state: State | null;
  city: City | null;
  major: Majors | null;
  language: LanguageStudy | null;
  academicDegree: string | null;
}

const ACADEMIC_DEGREES = {
  en: ["Bachelor's", "Master", "Ph.D", "Diploma", "Certificate"],
  ar: ["بكالوريوس", "ماجستير", "دكتوراه", "دبلومة", "شهادة"],
};
...
function NewSearch() {
  ...
  const router = useRouter();
  const dispatch = useDispatch<AppDispatch>();
  const [activeButton, setActiveButton] = useState<"UNI" | "Ins">("UNI");
  const [error, setError] = useState<boolean>(false);
  const [searchValue, setSearchValue] = useState<string>("");
  ...
  const languageStudy = useSelector(selectLanguageStudy);

  const filteredCountries = useMemo(() => {
    if (!search.country) return countries?.countries;
    return countries?.countries?.filter((c: any) =>
      c.name.toLowerCase().includes(search.country.toLowerCase())
    );
  }, [countries?.countries, search.country]);
  const filteredState = useMemo(() => {
    if (!search.state) return states?.states;
    return states?.states?.filter((c: any) =>
      c.name.toLowerCase().includes(search.state.toLowerCase())
    );
  }, [states?.states, search.state]);
  const filteredCities = useMemo(() => {
    if (!search.city) return cities?.cities;
    return cities?.cities?.filter((c: any) =>
      c.name.toLowerCase().includes(search.city.toLowerCase())
    );
  }, [cities?.cities, search.city]);
  const filteredLanguageStudy = useMemo(() => {
    if (!search.language) return languageStudy?.languageStudy;
    return languageStudy?.languageStudy?.filter((c: any) =>
      c.name.toLowerCase().includes(search.language.toLowerCase())
    );
  }, [languageStudy?.languageStudy, search.language]);
  const filteredMajors = useMemo(() => {
    if (!search.major) return majors?.majors;
    return majors?.majors?.filter((c: any) =>
      c.name.toLowerCase().includes(search.major.toLowerCase())
    );
  }, [majors?.majors, search.major]);
  ...
  const handleSubmit = () => {
    const formState = {
      ...
      academicDegree: selected?.academicDegree,
      majors: selected?.major?.id,
      search: searchValue,
      languageStudy: selected?.language?.id,
    };
    const hasAnyValue = Object.values(formState).some(
      (value) => value !== null && value !== undefined && value !== ""
    );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        setDropdowns(p => ({ ...p, country: false, AcademicDegree: false, major: false, state: false, city: false, language: false }))
      }
    };
          ...
          {activeButton === "UNI" ? (
            <div className="flex gap-3  w-full mt-4 max-md:flex-col max-md:gap-4 px-3 xl:ps-8">
              ...
              <div className="flex flex-col gap-2 items-start  lg:w-64  relative">
                ...
                <div
                  onClick={() => {
                    dispatch(getAllCountries({ language: language, entity: "universities" }));
                    langCountryRef.current = language;
                    setDropdowns(p => ({ ...p, country: !p.country }));
                    setSearch((p) => ({ ...p, country: "" }));
                  }}
                  className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100 h-10 rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3"
                >
                  <span className="text-base tracking-wide text-neutral-600 max-sm:text-sm">
                    {selected?.country?.name || n("searchFiltersDrd1")}
                  </span>
                  <svg
                    className={`transition-transform duration-500 ease-in-out ${dropdowns.country ? "rotate-180" : ""} `}
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  ...
                  <div className="absolute top-[90%] mt-2 w-full bg-white border border-zinc-100 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
                    ...
                    <div className="px-4 py-2">
                      <input
                        ...
                        onChange={(e) =>
                          setSearch((p) => ({ ...p, country: e.target.value }))
                        }
                      ...
                      filteredCountries?.length > 0 ? (
                        filteredCountries.map((country: Country) => (
                          <div
                            key={country?.id}
                            onClick={() => {
                              setSelected((p) => ({ ...p, country: country }));
                              setDropdowns(p => ({ ...p, country: !p.country }));
                            }}
                            className="px-6 py-3 hover:bg-gray-100 cursor-pointer"
                          >
              ...
              <div className="flex flex-col gap-2 items-start  lg:w-64  relative">
                ...
                <div
                  onClick={() => {
                    setDropdowns(p => ({ ...p, AcademicDegree: !p.AcademicDegree }))
                  }}
                  className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100 h-10 rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3 "
                >
                  <span className="tracking-wide text-neutral-600 text-sm">
                    {selected?.academicDegree || n("searchFilters2")}
                  </span>
                  <svg
                    className={`transition-transform duration-500 ease-in-out ${dropdowns.AcademicDegree ? "rotate-180" : ""} `}
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                {dropdowns.AcademicDegree && (
                  <div className="absolute top-[90%] mt-2 w-full bg-white border border-zinc-100 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
                    {language == "en"
                      ? ACADEMIC_DEGREES.en.map((academic: any, index: number) => (
                        <div
                          key={index}
                          onClick={() => {
                            setSelected((p) => ({ ...p, academicDegree: academic }));
                            setDropdowns(p => ({ ...p, AcademicDegree: !p.AcademicDegree }))
                          }}
                          className="px-6 py-3 hover:bg-gray cursor-pointer"
                        >
                      ...
                      : ACADEMIC_DEGREES.ar.map((academic: any, index: number) => (
                        <div
                          key={index}
                          onClick={() => {
                            setSelected((p) => ({ ...p, academicDegree: academic }));
              ...
              <div className="flex flex-col gap-2 items-start  lg:w-64  relative">
                ...
                <div
                  onClick={() => {
                    ...
                    setDropdowns(p => ({ ...p, major: !p.major }))
                  }}
                  className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100 h-10 rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3 "
                >
                  <span className="text-base tracking-wide text-neutral-600 max-sm:text-sm">
                    {selected?.major?.name || n("searchFilters3+")}
                  </span>
                  <svg
                    className={`transition-transform duration-500 ease-in-out ${dropdowns.major ? "rotate-180" : ""} `}
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  ...
                  <div className="absolute top-[90%] mt-2 w-full bg-white border border-zinc-100 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
                    <div className="px-4 py-2">
                      <input
                        ...
                        value={search?.major}
                        onChange={(e) =>
                          setSearch((p) => ({ ...p, major: e.target.value }))
                        }
                        className="w-full px-3 py-2 border border-gray rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    {!majors.loading ? (
                      filteredMajors?.length > 0 ? (
                        filteredMajors.map((major: Majors) => (
                          <div
                            key={major?.id}
                            onClick={() => {
                              // setSelectedMajors(major);
                              // setIsMajorsOpen(false);
                              setSelected((p) => ({ ...p, major: major }));
                              setDropdowns(p => ({ ...p, major: !p.major }))
                              // setSearchMajor("");
                              setSearch(p => ({ ...p, major: "" }))
                            }}
            ...
            <div className="flex gap-3 w-full mt-4 max-md:flex-col max-md:gap-4 px-3 xl:ps-8">
                        ...
                        filteredCountries.map((country: Country) => (
                          <div
                            ...
                            onClick={() => {
                              ...
                              setSearch((p) => ({ ...p, country: "" }))
                              setSelected((p) => ({ ...p, country: country }));
                              setSelected((p) => ({ ...p, state: null }));
                              setSelected((p) => ({ ...p, city: null }));
                              setDropdowns(p => ({ ...p, country: !p.country }))
              ...
              <div className="flex flex-col gap-2 items-start lg:w-60  relative">
                ...
                <div
                  onClick={() => {
                    setDropdowns(p => ({ ...p, state: !p.state }))
                  }}
                  className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100 h-10 rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3 "
                >
                  <span className="tracking-wide text-neutral-600 text-sm">
                    {selected?.state?.name || n("searchFiltersDrd2+")}
                  </span>
                  <svg
                    className={`transition-transform duration-500 ease-in-out ${dropdowns.state ? "rotate-180" : ""} `}
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                  ...
                  <div className="absolute top-[90%] mt-2 w-full bg-white border border-zinc-100 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
                    <div className="px-4 py-2">
                      <input
                        ...
                        value={search?.state}
                        onChange={(e) =>
                          setSearch((p) => ({ ...p, state: e.target.value }))
                        }
                        className="w-full px-3 py-2 border border-gray rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
              ...
              <div className="flex flex-col gap-2 items-start lg:w-60  relative">
                ...
                <div
                  onClick={() => {
                    setDropdowns(p => ({ ...p, city: !p.city }))
                  }}
                  className="flex justify-between items-center py-4 px-6 w-full border border-solid bg-neutral-50 border-zinc-100 h-10 rounded-[64px] cursor-pointer max-sm:px-4 max-sm:py-3 "
                >
                  <span className="tracking-wide text-neutral-600 text-sm">
                    {selected?.city?.name || n("searchFiltersDrd3+")}
                  </span>
                  <svg
                    className={`transition-transform duration-500 ease-in-out ${dropdowns.city ? "rotate-180" : ""} `}
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
              ...
              <div className="flex flex-col gap-2 items-start lg:w-60  relative">
                ...
                <div
                  onClick={() => {
                    ...
                    const currentCountryLang = langLanguageStudyRef.current !== language;
                    if (!languageStudy?.languageStudy?.length || currentCountryLang) {
                      dispatch(getLanguageStudy({ language: language }));
                      langLanguageStudyRef.current = language;
                    }
                ...
                >
                  <span className="text-base tracking-wide text-neutral-600 max-sm:text-sm">
                    {selected?.language?.name || n("searchFiltersDrd4+")}
                  </span>
                  <svg
                    className={`transition-transform duration-500 ease-in-out ${dropdowns.language ? "rotate-180" : ""} `}
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
                    fill="none"
                ...
                {dropdowns.language && (
                  <div className="absolute top-[90%] mt-2 w-full bg-white border border-zinc-100 rounded-xl shadow-lg z-50 max-h-60 overflow-y-auto">
                    ...
                    <div className="px-4 py-2">
                      <input
                        type="text"
                        placeholder={`${n("searchFiltersDrd4")}`}
                        value={search?.language}
                        onChange={(e) =>
                          setSearch((p) => ({ ...p, language: e.target.value }))
                        }
                        className="w-full px-3 py-2 border border-gray rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                      />
                    </div>
                    ...
                    {!languageStudy.loading ? (
                      filteredLanguageStudy?.length > 0 ? (
                        filteredLanguageStudy.map((language: LanguageStudy) => (
                          <div
                            key={language?.id}
                            onClick={() => {
                              setSearch((p) => ({ ...p, language: "" }))
                              setSelected(p => ({ ...p, language: language }))
                              setDropdowns(p => ({ ...p, language: !p.language }))
                            }}
                            className="px-6 py-3 hover:bg-gray-100 cursor-pointer"
                          >
