...
function NewMyDocs({ currentUser }: any) {
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const previewUrls = selectedFiles.map((file) => {
            if (file.type.startsWith("image/")) return { url: URL.createObjectURL(file), type: "image" };
            if (file.type === "application/pdf") return { url: "", type: "pdf" };
            return { url: "", type: "other" };
        });

        // replace selection (or switch to append if you want cumulative selection)
        setFiles(selectedFiles);
        setFilesPreview((prev) => [...prev, ...previewUrls]);
    ...
    const ConfirmDeletedDocs = async () => {
        ...
        toast.success(result.message || t("fileUdeleteSuccess"));
        dispatch(getUser({ tokenMainSite, locale }));
        setAlert(false);
        setIsLoading(false);
    }
