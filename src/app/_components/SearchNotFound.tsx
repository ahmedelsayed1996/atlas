...
function SearchNotFound({ head }: any) {
  const language = useCurrentLang();
  return (
    <div className="flex flex-col justify-center items-center self-stretch px-5 py-8 rounded-xl">
      <div className="flex justify-center items-center px-2.5 bg-gray-50 aspect-square rounded-[100px] w-[286px] max-md:px-5">
        {/* <Image src="/images/not-found.png" width={200} height={150} alt="Search" /> */}
        <svg width="150" height="170" viewBox="0 0 96 115" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="self-stretch text-xl lg:text-3xl font-bold text-center text-neutral-800 max-md:mt-10 max-md:max-w-full capitalize">

        {language === "en" ? `No ${head} found in search engines` : `لم يتم العثور على ${head} في محركات البحث`}
      </div>
      <div className="mt-7 text-base text-center text-zinc-500   max-md:max-w-full capitalize">
        {language === "en" ? `There is no ${head} with this name. Please search again for what you are looking for in a better way.` :
          `لا يوجد عنصر ${head} بهذا الاسم. يرجى البحث مرة أخرى عن ما تبحث عنه بطريقة أفضل.`}
      </div>
    </div>
  )
}
