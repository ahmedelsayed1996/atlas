...
import { LanguageSchool } from '../types/languageSchool';
import { RootState } from '../store';
import { FillStar } from './icons/Star';

interface SwiperSliderINSProps {
    data: {
        languageSchools?: {
            data: LanguageSchool[];
        };
...
export default function SwiperSliderINS({ data }: SwiperSliderINSProps) {
    ...
    const [languageSchools, setLanguageSchools] = useState<LanguageSchool[]>([]);


    const toggleSubscribe = async (
        languageSchoolId: number,
        isSubscribed: boolean
    ) => {
        setLoadingStates(prev => new Map(prev).set(languageSchoolId, true));
        ...
        try {
            ...
            setLanguageSchools(prev =>
                prev.map(lang =>
                    lang.id === languageSchoolId
                        ? { ...lang, is_notified: !isSubscribed }
                        : lang
                )
            );
            ...
            setLoadingStates(prev => new Map(prev).set(languageSchoolId, false));
        }
    };
    ...
    return (
        <div className="w-full py-10">
            <div className="flex flex-col md:flex-row justify-between items-center  max-sm:gap-5 px-5 md:px-12 lg:px-20 xl:py-10 xl:px-28">
                ...
                >
                    <span className="line-clamp-1"> {n("DiscoverButton")} </span>
                    <Arrow />
                </Link>
            </div>
                                        ...
                                        <SwiperSlide key={langSchool.id}>
                                            <div className="p-4 lg:p-6 bg-white rounded-3xl border border-solid border-zinc-100 ">
                                                <div className='flex justify-start items-center gap-3'>
                                                    <div className="flex justify-center items-center mb-4 rounded-lg border border-solid relative border-zinc-100 h-[130px] w-[145px] ">
                                                        <Image
                                                        ...
                                                        {langSchool.offer !== 0 && (
                                                            <div
                                                                dir={isAr ? "rtl" : "ltr"}
                                                                // flip angle for AR vs EN
                                                                // style={{ ["--angle" as any]: isAr ? "-45deg" : "45deg" }}
                                                                aria-label={`Offer ${langSchool.offer}%`}
                                                                className={[
                                                                    "bg-[#FBCC90] text-[#085D3A] text-sm absolute opacity-90 z-10",
                                                                    "flex items-center gap-2 px-2 py-1 rounded-lg overflow-hidden",
                                                                    "transform",
                                                                    "-top-3 z-10",
                                                                    "rotate-0 md:[transform:rotate(var(--angle))]",
                                                                ].join(" ")}
                                                            >
                                                                <svg
                                                                    width="10"
                                                                    ...
                                                                    focusable="false"
                                                                >
                                                                    <circle cx="5" cy="5" r="5" fill="#085D3A" />
                                                                </svg>
                                                    ...
                                                    <div className="flex flex-col gap-2 mb-4">
                                                        ...
                                                        {langSchool.recommend && (
                                                            <div
                                                                className={` text-sm flex items-center gap-2   py-1 rounded-lg ${language === "ar" ? "right-0" : "left-0"
                                                                    } top-0`}
                                                            >
                                                                <FillStar />
                                                ...
                                                <div className="flex gap-2 items-center mt-4">
                                                    {loadingStates.get(langSchool.id) ? <Spinner /> : <>
                                                        <Link
                                                            ...
                                                            className="flex gap-1 items-center px-4 py-0 h-10 text-base text-white text-nowrap cursor-pointer bg-primary rounded-[64px] justify-center grow"
                                                        >
                                                        ...
                                                        <div
                                                            ...
                                                            onClick={() => {
                                                                ...
                                                                toggleSubscribe(langSchool.id, langSchool.is_notified);
                                                            }}
                                                            className="flex justify-center items-center w-14 h-14  cursor-pointer rounded-[64px] "
                                                        >
                                                            <div>
                                                                {langSchool.is_notified ? <FillAlarm />
                                                                    : <SolidAlarm />
                                                                }
                                                            </div>
                                                        </div>
